...
namespace Sisus
{
	...
	public static class TypeExtensions
	{
		...
		/// </summary>
		public static Type[] GetExtendingUnityObjectTypes(this Type type, bool includeInvisible, bool includeAbstract = false)
		{
			#if DEV_MODE && PI_ASSERTATIONS
		...
		/// </summary>
		public static Type[] GetExtendingEditorTypes(this Type type, bool includeInvisible, bool includeAbstract = false)
		{
			#if DEV_MODE && PI_ASSERTATIONS
		...
		/// </summary>
		public static Type[] GetExtendingNonUnityObjectClassTypes(this Type type, bool includeInvisible, bool includeAbstract = false)
		{
			#if DEV_MODE && PI_ASSERTATIONS
						...
						do
						{
							if(testType.IsGenericType)
							{
								if(testType.IsGenericTypeDefinition)
								{
									...
									{
										#if DEV_MODE && PI_ASSERTATIONS
										Debug.Assert(!TypesList.Contains(type), "List already contained "+StringUtils.ToStringSansNamespace(type)+"!");
										#endif
										TypesList.Add(type);
									}
								}
						...
						do
						{
							...
							for(int n = 0, count = methods.Length; n < count; n++)
							{
								...
								if(method.IsInspectorViewable(methodVisibility, includeHidden))
								{
									...
									// Method overloads are also merged into one drawer to reduce clutter in Debug Mode+.
									if(!generatedMethods.Add(method.Name))
									{
										if(method.GetParameters().Length == 0) // <-- temp filter until method overload support added to MethodDrawer
										{
											continue;
										}
									}
