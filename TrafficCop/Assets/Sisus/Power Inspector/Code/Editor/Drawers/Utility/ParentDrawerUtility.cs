...
namespace Sisus
{
	...
	public static class ParentDrawerUtility
	{
		...
		/// </summary>
		public static bool ShowInInspector(IParentDrawer subject, bool passedLastFilterCheck)
        {
		...
		public static float GetOptimalPrefixLabelWidth(IParentDrawer subject, int indentLevel, bool prefixMayOverlapPrefixSeparator = true)
		{
			...
			{
				prefixWidth = DrawerUtility.GetOptimalPrefixLabelWidth(indentLevel, label, subject.HasUnappliedChanges);
			}
		...
		public static void SetUnfolded([NotNull]IParentDrawer subject, bool setUnfolded, bool setChildrenAlso)
		{
			...
			if(!setUnfolded && InspectorUtility.ActiveInspector.HasFilter)
			{
				#if DEV_MODE
		...
		public static void HandleErrorOrWarningBeforeControl([NotNull]GUIContent errorOrWarningLabel, Rect beforeControlPosition, bool isWarning)
		{
			if(errorOrWarningLabel.tooltip.Length > 0)
			{
				...
				if(GUI.Button(iconRect, errorOrWarningLabel, InspectorPreferences.Styles.Label))
				{
		...
		private static IDrawer GetNextSelectableDrawerUpInternal(IParentDrawer subject, int column, IDrawer requester, bool membersListedVertically)
		{
			...
			#endif
			return InspectorUtility.ActiveManager.FocusedControl;
		}
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, bool includeHidden, BindingFlags bindingFlags = BindingFlagsInstanceAndStatic)
		{
			...
			if(includeHidden)
			{
				GetMemberBuildList(parent, hierarchy, ref results, DebugModeDisplaySettings.DefaultSettings, bindingFlags);
				return;
			}
		...
		public static void GetMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, DebugModeDisplaySettings debugModeSettings, BindingFlags bindingFlags = BindingFlagsInstanceAndStatic)
		{
			...
			{
				parentType = type.IsUnityObject() ? LinkedMemberParent.UnityObject : LinkedMemberParent.Missing;
			}
			else
			{
				parentType = parentMemberInfo.IsStatic ? LinkedMemberParent.Static : LinkedMemberParent.LinkedMemberInfo;
			}
		...
		public static void GetStaticMemberBuildList([NotNull]IParentDrawer parent, [NotNull]LinkedMemberHierarchy hierarchy, [NotNull]ref List<LinkedMemberInfo> results, bool includeHidden, FieldVisibility fieldVisibility, PropertyVisibility propertyVisibility, MethodVisibility methodVisibility)
		{
			parent.Type.GetStaticInspectorViewables(ref hierarchy, parent.MemberInfo, ref results, includeHidden, fieldVisibility, propertyVisibility, methodVisibility);
		}
		...
		public static bool UpdateVisibleMembers(IParentDrawer subject)
		{
			...
			Debug.Log(StringUtils.ToColorizedString("UpdateVisibleMembers(", subject, ") called with MembersAreVisible=", subject.MembersAreVisible, ", Unfoldedness=", subject.Unfoldedness, ", Unfolded=", subject.Unfolded, ", DrawInSingleRow=", subject.DrawInSingleRow));
			#endif

			var members = subject.Members;

			#if DEV_MODE && PI_ASSERTATIONS
			if(members == subject.VisibleMembers && members.Length > 0)
			{
				Debug.LogError(subject.ToString()+" members array references same array as visible members - This can lead to bugs!\n"+StringUtils.ToString(members));
			}
			#endif

			int count = subject.Members.Length;
			...
			#if DEV_MODE && DEBUG_UPDATE_VISIBLE_MEMBERS_STEPS
			else { Debug.Log(subject + ".UpdateVisibleMembers() - MembersAreVisible was false"); }
			#endif
			...
			#if DEV_MODE && DEBUG_UPDATE_VISIBLE_MEMBERS
			if(subject.GetType() == typeof(NullableDrawer))
			Debug.Log(subject.GetType().Name+" / \""+subject.Name+"\".UpdateVisibleMembers() after: "+subject.VisibleMembers.Length+"/"+subject.Members.Length);
			#endif
