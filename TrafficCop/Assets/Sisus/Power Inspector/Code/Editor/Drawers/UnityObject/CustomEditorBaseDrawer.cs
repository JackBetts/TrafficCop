...
namespace Sisus
{
	...
	public abstract class CustomEditorBaseDrawer<TSelf, TTarget> : UnityObjectDrawer<TSelf, TTarget>, ICustomEditorDrawer where TSelf : CustomEditorBaseDrawer<TSelf, TTarget> where TTarget : Object
	{
		...
		private const int OffsetNextControlId = 20;
		...
		protected override bool UsesEditorForDrawingBody
		{
			get
			{
				try
				{
					return !DebugMode && !inspector.HasFilter;
				}
				#if DEV_MODE
		...
		{
			float result = MinAutoSizedPrefixLabelWidth;
			if(editor != null)
			{
				var serializedObjectField = editor.GetType().GetField("m_SerializedObject", BindingFlags.Instance | BindingFlags.NonPublic);
				var serializedObject = serializedObjectField.GetValue(editor) as SerializedObject;
				if(serializedObject != null)
				{
					...
					{
						enterChildren = false;
						tempLabel.text = iterator.displayName;
						float width = DrawerUtility.GetOptimalPrefixLabelWidth(indentLevel, tempLabel);
						if(width > result)
						{
				...
				result = Mathf.Clamp(result, MinPrefixLabelWidth, MaxPrefixLabelWidth);
			}
			return result;
		}
		...
		protected virtual float EstimatedUnfoldedHeight
		{
			get
			{
				...
				try
				{
					serializedObject = MemberHierarchy.SerializedObject;
				}
				#if DEV_MODE
				catch(Exception e)
				{
				...
				if(serializedObject != null)
				{
					...
					{
						estimatedHeight += DrawGUI.SingleLineHeight;
					}
					#if DEV_MODE && DEBUG_HEIGHT
					Inspector.OnNextLayout(()=>
					{
						Debug.Log(ToString()+" estimated height: "+estimatedHeight +" vs real height: "+Height);
					});
					#endif
					return estimatedHeight * 1.5f;
				}
				return HeaderHeight;
			}
		}
		...
		{
			if(filter.HasFilter && !memberBuildListActuallyPopulated)
			{
		...
		public override bool Draw(Rect position)
		{
			...
			//this should help make the default inspector look more similar
			//to like my own controls
			...
			//what about only drawing the header in inactive color if not enabled?
		...
		protected virtual bool DrawMembers(Rect position)
		{
			...
			GUILayout.BeginHorizontal();
			{
				...
				GUILayout.BeginVertical(style, GUILayout.Width(Width));
				{
					...
					if(eventType == EventType.Layout)
					{
						...
						#if DEV_MODE
						if(EndControlId <= controlId && EndControlId > 0)
						{
							Debug.LogError(ToString() + " - endControlID " + EndControlId + " < controlId "+ controlId+"! with Event.current.type="+Event.current.type);
							EndControlId = controlId + OffsetNextControlId + 100;
						}
						#endif
					}
			...
			{
				#if DEV_MODE
				Debug.Log(ToString()+ " GUI changed = true because usingInitialEstimatedHeight || NowUnfolding with Event=" + StringUtils.ToString(Event.current));
				#endif
				GUI.changed = true;
				dirty = true;
			}
		...
		private void OnInspectorGUI(Editor bodyEditor)
		{
			if(inspector == null)
			{
				return;
			...
			if(NowTweeningUnfoldedness)
			{
				...
				#if DEV_MODE && PI_ASSERTATIONS
				if(removeHeight >= 0f) { Debug.LogError("removeHeight ("+ removeHeight + ") >= 0f"); }
		...
		{
			//optimalPrefixLabelWidth = DrawGUI.DEFAULT_PREFIX_LABEL_WIDTH;
