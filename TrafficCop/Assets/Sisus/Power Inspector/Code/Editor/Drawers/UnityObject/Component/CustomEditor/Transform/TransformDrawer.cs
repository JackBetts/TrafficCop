...
namespace Sisus
{
	...
	public class TransformDrawer : ComponentDrawer, ITransformDrawer
	{
		...
		public RaycastHit?[] RaycastGround()
		{
			...
			var result = new RaycastHit?[count];
			for(int n = count - 1; n >= 0; n--)
			{
				var transform = unityObjects[n] as Transform;

				result[n] = null;
				if(transform != null)
				{
					var position = transform.position;
					var hits = Physics.BoxCastAll(position + Vector3.up, transform.localScale, Vector3.down);
					...
					for(int h = hits.Length - 1; h >= 0; h--)
					{
						...
						var point = hit.point;
						var distance = Vector3.SqrMagnitude(position - point);
