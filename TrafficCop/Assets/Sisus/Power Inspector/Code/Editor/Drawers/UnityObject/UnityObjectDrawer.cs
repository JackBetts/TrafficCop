...
//#define ENABLE_COMPONENTS_DISPLAY_NAMES
...
namespace Sisus
{
	...
	public abstract class UnityObjectDrawer<TSelf, TTarget> : ParentDrawer<LinkedMemberInfo>, IUnityObjectDrawer, IDebuggable where TSelf : UnityObjectDrawer<TSelf, TTarget> where TTarget : Object
	{
		...
		protected const float AssetToolbarIconsTopOffset = 4f;	
		...
		#else
		protected const float AssetHeaderToolbarIconWidth = 19f;
		protected const float AssetHeaderToolbarIconHeight = 17f;
		protected const float AssetToolbarIconsTopOffset = 5f;
		protected const float AssetHeaderToolbarIconsRightOffset = 0f;
		protected const float AssetHeaderToolbarIconsOffset = 0f;

		protected const float ComponentHeaderToolbarIconWidth = 18f;		
		protected const float ComponentHeaderToolbarIconHeight = 17f;
		protected const float ComponentToolbarIconsTopOffset = 2f;
		protected const float ComponentHeaderToolbarIconsRightOffset = 0f;
		protected const float ComponentHeaderToolbarIconsOffset = 0f;		
		#endif
		...
		#if DEV_MODE
		private Rect prefixResizerPosition; //to do: use this
		#endif
		...
		protected virtual bool Editable
		{
			get
			{
				...
				return GuiEnabled && (Target == null || !HasHideFlag(HideFlags.NotEditable));
			}
		}
		...
		private bool EditingComponentDisplayName
		{
			get
			{
				return IsComponent && HeaderIsSelected && DrawGUI.EditingTextField;
			}
		}
		#endif
		...
		protected virtual void Setup([NotNull]TTarget[] setTargets, [CanBeNull]IParentDrawer setParent, [CanBeNull]GUIContent setLabel, [NotNull]IInspector setInspector)
		{
			...
			Debug.Assert(headerButtons.Count == 0);
			Debug.Assert(headerParts.Count == 0);
			#endif
		...
		public override bool Draw(Rect position)
		{
			...
			{
				#if DEV_MODE
				Debug.LogError(this+".Draw() - target was null, rebuilding");
				#endif
		...
		public override bool DrawPrefix(Rect position)
		{
			...
			beforeHeaderControlId = GUIUtility.GetControlID(FocusType.Keyboard);
		...
		protected virtual void DrawHeaderBase(Rect position)
		{
			...
			#if UNITY_EDITOR && DEV_MODE && ENABLE_COMPONENTS_DISPLAY_NAMES
			if(Event.current.type == EventType.Repaint || EditingComponentDisplayName)
			{
				var labelRect = position;
				labelRect.x += 55f;
				labelRect.y += 1f;
				labelRect.height = 19f;
				labelRect.width -= 55f + (position.width - prefixIconBackgroundPosition.x);

				var components = targets as Component[];
				bool mixedContent;
				string displayNameWas = ComponentDisplayName.Get(components, out mixedContent);
				if(mixedContent)
				{
					EditorGUI.showMixedValue = true;
				}

				var labelRectBg = labelRect;
				labelRectBg.x -= 5f;
				labelRectBg.width += 10f;
				DrawGUI.Active.ColorRect(labelRectBg, PrefixBackgroundColor);

				if(EditingComponentDisplayName)
				{
					if(!string.Equals(GUI.GetNameOfFocusedControl(), "Edit Component Display Name"))
					{
						GUI.SetNextControlName("Edit Component Display Name");
						GUI.FocusControl("Edit Component Display Name");
					}
					string setDisplayName = EditorGUI.TextField(labelRect, displayNameWas);//, InspectorPreferences.Styles.TitleText);
				
					if(!string.Equals(setDisplayName, displayNameWas))
					{
						#if DEV_MODE
						Debug.Log("Set display name: "+setDisplayName);
						#endif

						ComponentDisplayName.Set(components, setDisplayName);
					}
				}
				else
				{
					string displayNameWithType = ComponentDisplayName.GetWithType(components[0]);
					GUI.Label(labelRect, displayNameWithType, InspectorPreferences.Styles.TitleText);
				}
		...
		private void OpenDocumentation(IUnityObjectDrawer containingDrawer, Rect buttonRect, Event inputEvent)
		{
			...
			string fullUrl;
			if(urlBase.StartsWith("unity/", StringComparison.Ordinal))
			{
		...
		/// <param name="mouseovereredPart"> [out] The part of the header that is now being mouseoverered. </param>
		/// <returns> True if cursor is over any header part, otherwise false. </returns>
		private bool GetMouseoveredHeaderPart(out HeaderPartDrawer mouseovereredPart)
		{
			for(int n = headerParts.Count - 1; n >= 0; n--)
			{
				var part = headerParts[n];
				if(part.MouseIsOver())
				{
					...
					mouseovereredPart = part;
					return true;
				}
			}
			mouseovereredPart = null;
			return false;
		}
		...
		protected Rect GetRectForFirstHeaderButtonEnd(Rect headerRect)
		{
			...
			pos.height = 18f;
			#else
			pos.x = headerRect.width - Buttons.ButtonDrawOffset;
			pos.y += 25f;
			pos.height = 15f;
			#endif
			return pos;
		...
		public override void OnMouseover()
		{
			if(HeaderMouseovered)
			{
				...
				#if !UNITY_2019_3_OR_NEWER
				if(InspectorUtility.Preferences.mouseoverEffects.headerButton)
				{
					DrawGUI.DrawLeftClickAreaMouseoverEffect(mouseoveredRect, localDrawAreaOffset);
				}
				#endif
...
DrawGUI.DrawLeftClickAreaMouseoverEffect(mouseoveredRect, localDrawAreaOffset);
		...
		public override void OnMouseUpAfterDownOverControl(Event inputEvent, bool isClick)
		{
			...
			Debug.Log(Msg(GetType(), ".OnMouseUpAfterDownOverControl(isClick=", isClick, ") with MouseoveredHeaderPart=", MouseoveredHeaderPart, ", HeaderMouseovered=", HeaderMouseovered, ", Selectable=", Selectable, ", SelectedHeaderPart=", (HeaderPart)SelectedHeaderPart, ", event = ", StringUtils.ToString(inputEvent), ", mousePos=", inputEvent.mousePosition, "CanBeSelectedWithoutHeaderBeingSelected=", CanBeSelectedWithoutHeaderBeingSelected));
			#endif
			...
			if(isClick)
			{
				...
				if(selectedWas == HeaderPart.FoldoutArrow || selectedWas == HeaderPart.Base)
				{
				...
				if(mouseoveredPart == headerParts[HeaderPart.Base])
				{
					if(mouseovereredHeaderButton != null)
					{
						DrawGUI.Use(inputEvent);
		...
		public virtual void AddItemsToOpeningViewMenu(ref Menu menu)
		{
			...
			#else
			ViewMenuUtility.AddFieldVisibilityItems(ref menu, inspector, false);
			#endif
		...
		#if UNITY_EDITOR
		/// <summary> TO DO: support missing components! </summary>
		public void MoveUp()
		{
			#if DEV_MODE && PI_ASSERTATIONS
		...
		public void ApplyDebugModeSettings(DebugModeDisplaySettings settings)
		{
			...
			var selectedControl = inspector.Manager.FocusedControl;
			...
			{
				for(var nextParent = parent; nextParent != null; nextParent = nextParent.Parent)
				{
					drawerGroup = parent as  DrawerGroup;
					if(drawerGroup != null)
					{
						drawerGroup.SetUnfoldedTemporarilyForFiltering(this);
					}
				}
			}
			...
			if(drawerGroup != null)
			{
				drawerGroup.RestoreFoldedStateWhenDisposingDuringFiltering(this);
			}
		...
		public override bool OnKeyboardInputGiven(Event inputEvent, KeyConfigs keys)
		{
			...
			#if UNITY_EDITOR && DEV_MODE && ENABLE_COMPONENTS_DISPLAY_NAMES
			else if(EditingComponentDisplayName)
			{
		...
		public override bool OnClick(Event inputEvent)
		{
			...
			if(mouseovereredHeaderButton != null)
			{
				mouseovereredHeaderButton.OnClicked();
				DrawGUI.Use(inputEvent);
				return true;
			}
		...
		protected override void DoReset()
		{
			...
			#if UNITY_EDITOR && UNITY_2018_1_OR_NEWER // Presets were introduced in Unity 2018.1
			...
			{
				#if UNITY_EDITOR && UNITY_2019_3_OR_NEWER // Preset.GetDefaultForObject was deprecated and superceded by Preset.GetDefaultPresetsForObject in Unity 2019.3
				...
				#else
				var defaultPreset = UnityEditor.Presets.Preset.GetDefaultForObject(Target);
				#endif
		...
		protected virtual bool HasHideFlag(HideFlags hideFlag)
		{
			for(int n = targets.Length - 1; n >= 0; n--)
			{
				if(targets[n].hideFlags.HasFlag(hideFlag))
				{
					return true;
				}
			}
