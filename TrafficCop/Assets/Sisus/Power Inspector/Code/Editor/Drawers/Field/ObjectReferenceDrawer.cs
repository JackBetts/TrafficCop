...
namespace Sisus
{
	...
	public class ObjectReferenceDrawer : PrefixControlComboDrawer<Object>
	{
			...
			if(drawEyedropper)
			{
				if(Event.current.type == EventType.Repaint)
				{
					...
					if(DrawerUtility.GUIColorHistory.Count > 0 && !DataIsValid)
					{
						...
						DrawGUI.Active.ColorRect(objectFieldRectIncludingPicker, DrawGUI.Active.InspectorBackgroundColor);
		...
		{
			#if DEV_MODE
			Debug.Log("AcceptableDragNDropSubjects(" + StringUtils.TypesToString(dragAndDropObjectReferences) + ")");
			#endif
