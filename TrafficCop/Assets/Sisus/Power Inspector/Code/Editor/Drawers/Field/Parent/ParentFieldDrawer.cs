...
namespace Sisus
{
	...
	public abstract class ParentFieldDrawer<TValue> : FieldDrawer<TValue>, IParentDrawer
	{
		...
		protected override void OnCachedValueChanged(bool applyToField, bool updateMembers)
		{
			...
			#if DEV_MODE && PI_ASSERTATIONS
			if(!Inspector.HasFilter && MembersAreVisible && !updateCachedValuesFor.ContentsMatch(visibleMembers.Where(drawer => drawer.CachedValuesNeedUpdating).ToList(), false))
			{
			...
			try
			{
				...
				if(RebuildDrawersIfValueChanged)
				{
					...
					var selected = inspector.Manager.FocusedControl;
		...
		private void ClearMemberBuildList()
		{
			...
			{
				var memberBuildInfo = memberBuildList[n];

				#if DEV_MODE
				Debug.Assert(memberBuildInfo != null, ToString()+ ".memberBuildList["+n+"] was null! memberBuildList.Count="+memberBuildList.Count+", members.Length="+members.Length);
				#endif

				hierarchy.Dispose(ref memberBuildInfo);
			}
		...
		protected virtual void DoBuildMembers()
		{
			...
			#if DEV_MODE // TEMP
			if(GetType() == typeof(DataSetDrawer)) { Debug.Log(ToString() + ".DoBuildMembers members.Count now: " + members.Length); }
			#endif
		}
		...
		public override void UpdateCachedValuesFromFieldsRecursively()
		{
			...
			//Update cached values on self from field
			UpdateCachedValueFromField(RebuildDrawersIfValueChanged);
		}
		...
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(position.width > 0f, position.ToString());
			#endif

			//needed because of the foldout control
			GenerateControlId();
		...
		private void DrawDragBarIfReorderable()
		{
			if(IsReorderable)
			{
		...
		protected override void OnLayoutEvent(Rect position)
		{
			...
			foldArrowMouseovered = Foldable && Mouseovered && FoldArrowPosition.Contains(Event.current.mousePosition); //FoldArrowPosition.MouseIsOver());
		}
