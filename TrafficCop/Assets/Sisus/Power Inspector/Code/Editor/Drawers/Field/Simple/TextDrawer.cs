...
namespace Sisus
{
	...
	public class TextDrawer : PrefixControlComboDrawer<string>, ITextFieldDrawer, IPropertyDrawerDrawer
	{
		...
		public override string DrawControlVisuals(Rect position, string inputValue)
		{
			...
			if(position.height > DrawGUI.SingleLineHeight)
			{
				InspectorUtility.Preferences.GUISkin.textArea.wordWrap = HeightDeterminant == TextFieldHeightDeterminant.WordWrapping;
				inputValue = DrawGUI.Active.TextArea(position, inputValue);
				InspectorUtility.Preferences.GUISkin.textArea.wordWrap = true;
				return inputValue;
			}
