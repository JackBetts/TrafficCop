...
namespace Sisus
{
	...
	public abstract class FieldDrawer<TValue> : BaseDrawer, IFieldDrawer
	{
		...
		protected virtual void Setup([CanBeNull]TValue setValue, [NotNull]Type setValueType, [CanBeNull]LinkedMemberInfo setMemberInfo, [CanBeNull]IParentDrawer setParent, [CanBeNull]GUIContent setLabel, bool setReadOnly)
		{
			...
			else if(setValueType.IsAbstract || Nullable.GetUnderlyingType(setValueType) != null)
			{
				canBeNull = true;
			}
			#if UNITY_2019_3_OR_NEWER
			else if(setMemberInfo != null && (setMemberInfo.GetAttribute<SerializeReference>() != null || setMemberInfo.GetAttribute<CanBeNullAttribute>() != null))
			#else 
			else if(setMemberInfo != null && setMemberInfo.GetAttribute<CanBeNullAttribute>() != null)
			#endif
			{
				canBeNull = true;
			}
			...
			{
				Debug.LogWarning(StringUtils.ToColorizedString(ToString(), ".Setup - readonly=", false, " but parent.ReadOnly=", true, ". Intentional?"));
			}
			#endif
			...
			if(setValue == null && !CanBeNull)
			{
				...
				{
					Debug.Log(Msg(ToString(setLabel, setMemberInfo), ".Setup - setValue is ", null, " with setMemberInfo=", setMemberInfo, ", CanWriteToFieldWithoutSideEffects =", CanWriteToFieldWithoutSideEffects, ", IsStatic=", setMemberInfo.IsStatic, ", IsGenericType=", setMemberInfo.Type.IsGenericType, ", IsGenericParameter=", setMemberInfo.Type.IsGenericParameter, ", IsGenericTypeDefinition=", setMemberInfo.Type.IsGenericTypeDefinition));
				}
				else
				{
					Debug.Log(Msg(ToString(setLabel, setMemberInfo), ".Setup - setValue is ", null, " with setMemberInfo=", null, ", CanWriteToFieldWithoutSideEffects =", CanWriteToFieldWithoutSideEffects));
				}
				#endif
				...
				#if DEV_MODE && DEBUG_NULL_VALUE
				Debug.Log(Msg(ToString(), ".Setup - setValue from ", null, " to  ", setValue, " with setMemberInfo=", setMemberInfo, ", CanWriteToFieldWithoutSideEffects =", CanWriteToFieldWithoutSideEffects, ", IsStatic=", (setMemberInfo == null ? false : setMemberInfo.IsStatic)));
				#endif
		...
		protected virtual bool DoSetValue(TValue setValue, bool applyToField, bool updateMembers)
		{
			...
			Debug.Log(Msg(ToString(), ".Value = ", setValue, " (was: ", value, ") with applyToField=", applyToField, ", updateMembers=", updateMembers, ", ValueEquals(setValue)=", MixedContent ? StringUtils.Green("(WasMixed)") : StringUtils.ToColorizedString(ValueEquals(setValue))));
			#endif

			bool mixedContent = MixedContent;

			if((mixedContent && applyToField) || (!mixedContent && !ValueEquals(setValue)))
			{
				value = setValue;
		...
		protected bool UpdateCachedValueFromField(bool updateMembers)
		{
			if(!CanReadFromFieldWithoutSideEffects)
			{
				return false;
			...
			catch(Exception e)
			{
				...
				if(e is InvalidCastException)
				{
					...
					Debug.LogError(ToString() + ".UpdateCachedValueFromField Could not cast GetValue(0) " + StringUtils.ToString(getValue) + " of type " + StringUtils.TypeToString(getValue) + " to type " + typeof(TValue).Name + "\n" + e + "\nType=" + Type + ", HasLinkedMemberInfo=" + (memberInfo != null ? "True" : "False"));
				}
				else
				{
		...
		protected override void BuildContextMenu(ref Menu menu, bool extendedMenu)
		{
			...
			#if DEV_MODE // still experimental
			if(canBeNull)
			{
				if(Value == null)
				{
					menu.Add("Set Null", () => SetValue(null));
				}
				else if(!Type.IsAbstract && !Type.IsUnityObject() && Type != Types.SystemObject && Type != Types.Enum)
				{
					menu.Add("Set Not Null", ()=>SetValue(null));
				}
			}
			#endif
			...
			if(canWrite)
			{
				...
				if(CanReset)
				{
					if(!ValueEquals((TValue)DefaultValue()))
					{
			...
			if(canWrite)
			{
				...
				if(DebugMode || Event.current.control)
				{
					menu.AddSeparatorIfNotRedundant();
		...
		/// <inheritdoc/>
		protected override void AddDebuggingEntriesToRightClickMenu(ref Menu menu)
		{
			base.AddDebuggingEntriesToRightClickMenu(ref menu);

			bool hasLogPoint = HasOnValueChangedLogPoint();
			bool hasBreakPoint = HasOnValueChangedBreakPoint();
			if(!ReadOnly || hasLogPoint || hasBreakPoint)
			{
				if(!menu.ContainsCategory("Debugging"))
				{
					menu.AddSeparatorIfNotRedundant();
				}
				...
				if(hasLogPoint)
				{
					menu.Add("Debugging/Log Point", "Log value changes to console.", RemoveOnValueChangedLogPoint, true);
				}
				else
				{
					menu.Add("Debugging/Log Point", "Log value changes to console.", AddOnValueChangedLogPoint, false);
				}

				if(hasBreakPoint)
				{
					menu.Add("Debugging/Break Point", "If value changes log the value and pause the editor.", RemoveOnValueChangedBreakPoint, true);
				}
				else
				{
					menu.Add("Debugging/Break Point", "If value changes log the value and pause the editor.", AddOnValueChangedBreakPoint, false);
				}
			}
		}

		private void AddCopyMenuItems(ref Menu menu)
		{
			...
			{
				AddCopyToClipboardMenuItemsForMixedContent(ref menu);
			}
		}
		...
		private void AddCopyToClipboardMenuItemsForMixedContent(ref Menu menu)
		{
			var values = GetValues();
		...
		protected virtual void AddMenuItemsFromAttributes(ref Menu menu)
		{
			...
			var items = memberInfo.GetAttributes<ContextMenuItemAttribute>();
