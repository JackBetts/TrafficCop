...
namespace Sisus
{
	...
	public abstract class BaseDrawer : IDrawer, IDisposable, IEquatable<BaseDrawer>
	{
		...
		public virtual Object[] UnityObjects
		{
			get
			{
				//could help some Drawer like header
				if(parent != null)
				{
		...
		public virtual bool Draw(Rect position)
		{
			...
			switch(Event.current.type)
			{
				...
				case EventType.Repaint:
					break;
				default:
					//don't waste resources drawing the prefix
					//if it's not a Repaint or Layout event?
					return false;
			}
		...
		protected void GenerateControlId()
		{
		...
		{
			var manager = Manager;
			if(manager != null && manager.FocusedControl == this)
			{
		...
		public virtual IDrawer GetNextSelectableDrawerLeft(bool moveToNextControlAfterReachingEnd, IDrawer requester)
		{
			...
			if(Manager.FocusedControl == this || !Selectable)
			{
				#if DEV_MODE && DEBUG_NEXT_FIELD
		...
		public void HandleOnClickSelection(Event inputEvent, ReasonSelectionChanged reason)
		{
			...
			if(Selectable)
			{
				...
				{
					var focusedControl = Inspector.Manager.FocusedControl;
		...
		{
			bool extendedMenu = Event.current.control || DebugMode;
			...
			if(extendedMenu)
			{
				AddDebuggingEntriesToRightClickMenu(ref menu);
			}
		...
		protected virtual void BuildContextMenu(ref Menu menu, bool extendedMenu)
		{
			...
			if(inspector != null && inspector.HasFilter)
			{
		...
		protected void AddOnValueChangedBreakPoint()
		{
			onValueChanged += OnValueChangedBreakPoint;
		}

		protected void RemoveOnValueChangedBreakPoint()
		{
			onValueChanged -= OnValueChangedBreakPoint;
		}

		protected void OnValueChangedBreakPoint(IDrawer changed, object value)
		{
			Debug.Log("<size=22>[BREAK POINT] " + changed+" value changed to "+StringUtils.ToString(value)+ "</size>");
			Debug.Break();
		}

		protected bool HasOnValueChangedLogPoint()
		{
			if(onValueChanged == null)
			{
				return false;
			...
			{
				var method = list[n].Method;
				if(method != null && string.Equals(method.Name, "OnValueChangedLogPoint"))
				{
					return true;
				}
			}
		...
		protected void AddOnValueChangedLogPoint()
		{
			onValueChanged += OnValueChangedLogPoint;
		}

		protected void RemoveOnValueChangedLogPoint()
		{
			onValueChanged -= OnValueChangedLogPoint;
		}

		protected void OnValueChangedLogPoint(IDrawer changed, object value)
		{
			Debug.Log("<size=22>[LOG POINT] " + changed+" value changed to "+StringUtils.ToString(value)+ "</size>");
		}

		/// <summary>
		/// Adds debugging entries meant for debugging to opening right click menu.
		/// Invoked when control is right clicked with the control key is held down or Debug Mode+ is enabled.
		/// </summary>
		/// <param name="menu"> [in,out] The opening menu into which to add the menu items. </param>
		protected virtual void AddDebuggingEntriesToRightClickMenu(ref Menu menu) {	}
		...
		/// <summary> Updates the dataIsValid fields value based on whether or not the field's data is
		/// valid. Uses either overrideValidateValue if it is not null, or otherwise uses the method
		/// GetDataIsValidUpdated()
		/// to get the value for the field. </summary>
		protected virtual void UpdateDataValidity(bool evenIfCanHaveSideEffects)
		{
			if(overrideValidateValue != null)
			{
				if(!evenIfCanHaveSideEffects) 
				{
					#if DEV_MODE
					Debug.LogWarning("Ignoring "+ToString()+".UpdateDataValidity("+ evenIfCanHaveSideEffects+ " with overrideValidateValue="+StringUtils.ToString(overrideValidateValue)+" because evenIfCanHaveSideEffects as false");
					#endif
					return;
				}
				...
				if(TryGetSingleValueVisualizedInInspector(out visualizedValue))
				{
					...
					#if DEV_MODE
					Debug.Log(ToString()+ ".UpdateDataValidity with visualizedValue=" + StringUtils.TypeToString(visualizedValue)+ ", GetValue()=" + StringUtils.TypeToString(GetValue())+ " dataIsValid: "+ dataIsValid);
					#endif
				}
				else
				{
					#if DEV_MODE
					Debug.Log(ToString()+ ".UpdateDataValidity with GetValues()="+ StringUtils.TypesToString(GetValues()));
					#endif
		...
		protected void ClearFilterAndScrollToShow()
		{
			var inspector = Inspector;
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(inspector.HasFilter);
			#endif
