...
namespace Sisus
{
	public abstract class DrawGUI
	{
		...
		public const float MinAutoSizedPrefixLabelWidth = 35f;
		...
		public static GUIStyle prefixLabelSelectedMouseovered;
		public static GUIStyle prefixLabelSelectedMouseoveredUnfocused;
		public static GUIStyle prefixLabelMouseoveredModified;
		public static GUIStyle prefixLabelMouseoveredModifiedUnfocused;
		...
		static DrawGUI()
		{
			...
			//OnNextBeginOnGUI(()=>ApplyPlayModeTinting(EditorApplication.isPlayingOrWillChangePlaymode), false);
		}
		...
		public static bool ActionKey
		{
			get
			{
				...
				#if UNITY_STANDALONE_OSX || UNITY_IOS || UNITY_IPHONE
				return e.command;
				#else
				return e.control;
				#endif
		...
		public static int IndentLevel
		{
			...
			set
			{
				...
				#if DEV_MODE && DEBUG_SET_INDENT_LEVEL && UNITY_EDITOR
		...
		public abstract bool ShowMixedValue
		{
			get;
			set;
		}
		...
		public static bool EditingTextField
		{
			get
			{
				...
				if(EditorGUIUtility.editingTextField != editingTextField && InspectorUtility.ActiveManager != null && InspectorUtility.ActiveManager.SelectedInspector != null)
				{
			...
			set
			{
				...
				if(editingTextField != value) { Debug.Log(StringUtils.ToColorizedString("EditingTextField = ", value, " (with KeyboardControl=", KeyboardControlUtility.KeyboardControl, ")")); }
				#endif
				...
				if(InspectorUtility.ActiveManager != null && InspectorUtility.ActiveManager.SelectedInspector != null)
				{
					...
					{
						#if DEV_MODE && PI_ASSERTATIONS
						if(EditorWindow.focusedWindow != null && !ReferenceEquals(EditorWindow.focusedWindow, InspectorUtility.ActiveInspectorDrawer) && !PopupMenu.isOpen && !AddComponentMenuWindow.IsOpen)
						{ Debug.LogWarning("EditingTextField = " + value + " called with focusedWindow ("+ EditorWindow.focusedWindow + ") != InspectorUtility.ActiveDrawer ("+ InspectorUtility.ActiveInspectorDrawer + ")"); }
						#endif

						#if DEV_MODE && DEBUG_SET_EDITING_TEXT_FIELD
						if(editingTextField != value) { Debug.Log(StringUtils.ToColorizedString("EditorGUIUtility.editingTextField = ", value, " (with EditorWindow.focusedWindow=", EditorWindow.focusedWindow, ")")); }
						#endif
				...
				#if DEV_MODE && DEBUG_SET_EDITING_TEXT_FIELD
				else if(EditorGUIUtility.editingTextField != value) { Debug.LogWarning("Won't sync EditorGUIUtility.editingTextField with EditingTextField because no inspector was selected"); }
				#endif
				#else
				Debug.LogError("EditingTextField not yet supported in play mode!");
				#endif
			}
		}
		...
		protected static void SendOnDragAndDropObjectReferencesChangedEventIfNeeded(bool calledBySetDragAndDropObjectReferences)
		{
			...
			{
				if(!InspectorUtility.ActiveManager.MouseDownInfo.MouseButtonIsDown)
				{
					// UPDATE: This sometimes messed with externally started Object dragging
					//InspectorUtility.ActiveManager.OnNextLayout(()=> Active.DragAndDropObjectReferences = ArrayPool<Object>.ZeroSizeArray);
				}
		...
		public static void FocusControl(string internalName)
		{
			...
			Debug.Assert(!InspectorUtility.ActiveManager.HasMultiSelectedControls || internalName.Length == 0);
			Debug.Assert(!ObjectPicker.IsOpen);
			Debug.Assert(EditorWindow.focusedWindow == InspectorUtility.ActiveManager.GetLastSelectedEditorWindow());
			#endif
		...
		public static void OnEveryBeginOnGUI([NotNull]Action action, bool ensureOnGUICallbacks)
		{
			...
			{
				EnsureOnGUICallbackOnExistingInspector();
			}
		}
		...
		{
			#if DEV_MODE && UNITY_EDITOR && (PI_ASSERTATIONS || DEBUG_ENSURE_ON_GUI_CALLBACKS)
			if(InspectorManager.InstanceExists())
			{
				#if DEBUG_ENSURE_ON_GUI_CALLBACKS
				Debug.Log(StringUtils.ToColorizedString("EnsureOnGUICallbacks with InspectorManager.InstanceExists()=", true, ", ", InspectorManager.instance.ActiveInstances.Count, " ActiveInstances=", StringUtils.ToString(InspectorManager.instance.ActiveInstances)));
				#endif

				#if PI_ASSERTATIONS
				for(int n = InspectorManager.instance.ActiveInstances.Count - 1; n >= 0; n--)
				{
					var instance = InspectorManager.instance.ActiveInstances[n];
					if(!instance.SetupDone) { Debug.LogWarning("EnsureOnGUICallbacks called with active instances existing in scene. However SetupDone was false for instance "+(n+1)+"/"+ InspectorManager.instance.ActiveInstances.Count); }
					Debug.Assert(instance.InspectorDrawer != null);
					Debug.Assert(instance.InspectorDrawer as EditorWindow == null || ((EditorWindow)instance.InspectorDrawer).IsVisible());
				}
				#endif
			}
			#endif
			...
			#if UNITY_EDITOR
			if(!InspectorManager.InstanceExists() || InspectorManager.instance.ActiveInstances.Count == 0)
			{
		...
		public static void EnsureOnGUICallbackOnExistingInspector()
		{
			var manager = InspectorUtility.ActiveManager;
			if(manager != null)
			{
		...
		public static void DrawEdgeSelectionIndicator(Rect rect)
		{
			...
			rect.y -= 1f;
			rect.height += 1f;
			Platform.Active.GUI.ColorRect(rect, GetSelectedLineIndicatorColor());
		}
		...
		public static void DrawSelectionOrFocusedControlRect(Rect rect, Color color)
		{
			if(rect.x <= 16f)
			{
				...
				rect.y -= 1f;
				rect.height += 1f;
				Platform.Active.GUI.ColorRect(rect, color);
			}
			else
			{
				DrawRect(rect, color);
		...
		public abstract bool Button(Rect position, GUIContent label);
		public abstract bool Button(Rect position, GUIContent label, [NotNull]GUIStyle guiStyle);
		...
		public abstract string TextArea(Rect position, string value);
