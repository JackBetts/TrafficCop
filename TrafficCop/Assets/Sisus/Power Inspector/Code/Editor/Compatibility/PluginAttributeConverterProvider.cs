		...
		static PluginAttributeConverterProvider()
		{
			...
			for(int n = count - 1; n >= 0; n--)
			{
				...
				try
				{
					...
					Debug.Assert(typeof(PluginAttributeConverterProvider).IsAssignableFrom(allTypes[n]), allTypes[n].FullName);
					#endif

					converterProvider = (PluginAttributeConverterProvider)Activator.CreateInstance(allTypes[n]);
				}
				catch(Exception e)
				{
					Debug.LogError("Failed to create instance of "+ allTypes[n].FullName+ " and cast it to PluginAttributeConverterProvider: " + e);
					continue;
				}
