...
namespace Sisus
{
	...
	public abstract class Inspector<TInspectorDrawer, TToolbar> : IInspector where TInspectorDrawer : class, IInspectorDrawer where TToolbar : IInspectorToolbar, new()
	{
		...
		public bool RebuildDrawers(Object[] targets, bool evenIfTargetsTheSame)
		{
			...
			{
				#if DEV_MODE
				Debug.Log("Ignoring RebuildDrawers call because DefaultDrawerProviders.IsReady was false.");
				#endif
				return false;
			}
			...
			else
			{
				...
				else
				{
					...
					if(count == 1 && firstTarget != null)
					{
						string assetPath = UnityEditor.AssetDatabase.GetAssetPath(firstTarget);
						...
						{
							switch(firstTarget.GetType().FullName)
							{
								case "UnityEditor.MonoManager":
									#if DEV_MODE
									Debug.LogWarning("RebuildDrawers Abort - Settings asset detected: " + firstTarget.GetType().FullName);
									#endif
							...
							#if DEV_MODE
							Debug.LogWarning("RebuildDrawers - Non-GameObject with no asset path detected, but probably not a settings object: " + firstTarget.GetType().FullName);
							#endif
						}
					}
					#endif
			...
			if(!HasFilter && UserSettings.EditComponentsOneAtATime && inspectedTargetsChanged && !UnityObjectDrawerUtility.HeadlessMode && allAreGameObjects)
            {
                FoldAllComponents();
            }
		...
		public void BroadcastOnFilterChanged()
		{
			...
			var selected = Manager.FocusedControl;
