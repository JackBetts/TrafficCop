...
namespace Sisus
{
	...
	public class SearchBoxToolbarItem : ToolbarItem, ISearchBoxToolbarItem
	{
		...
		protected override void UpdateDrawPositions(Rect itemPosition)
		{
			...
			filterFieldDrawRect.width -= ClearButtonWidth; //in Unity 2019.3's modern UI the clear button is drawn inside the filter field, while in previous versions it's drawn right after it.
			#endif
		...
		/// <inheritdoc/>
		protected override void OnGUI(Rect itemPosition)
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(!inspector.State.WantsSearchBoxDisabled);
			#if UNITY_EDITOR
			Debug.Assert(DrawGUI.EditingTextField == UnityEditor.EditorGUIUtility.editingTextField, "DrawGUI.EditingTextField ("+DrawGUI.EditingTextField+") != EditorGUIUtility.editingTextField ("+UnityEditor.EditorGUIUtility.editingTextField+")");
			#endif
			#endif
			...
			if(mouseoveredPart != SearchBoxPart.None)
			{
		...
		protected override bool OnActivated(Event inputEvent, bool isClick)
		{
			if(isClick)
			{
				switch(mouseoveredPart)
				{
					...
					default:
						#if DEV_MODE
						throw new System.IndexOutOfRangeException(mouseoveredPart.ToString());
						#else
						DrawGUI.Use(inputEvent);
						return false;
						#endif
