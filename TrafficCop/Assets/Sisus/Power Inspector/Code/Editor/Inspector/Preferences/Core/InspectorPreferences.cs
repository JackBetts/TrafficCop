...
namespace Sisus
{
	...
	public class InspectorPreferences : ScriptableObject
	{
		...
		private static AssetDrawer preferencesDrawer;
		...
		public GUIStyle GetStyle(string styleName)
		{
			try
			{
				return GUISkin.GetStyle(styleName);
			}
			#if DEV_MODE
			catch(NullReferenceException e) 
			{
				Debug.LogError("InspectorPreferences.GetStyle("+StringUtils.ToString(styleName)+") NullReferenceException with isCompiling="+Platform.IsCompiling+".\n"+e);
			#else
			catch(NullReferenceException)
			{
			#endif
