	...
	public class LinkedMemberInfo
	{
		...
		public void SetupCollectionMember([NotNull]LinkedMemberInfo setParent, Type type, int collectionIndex, GetCollectionMember getDelegate, SetCollectionMember setDelegate, string setSerializedPropertyRelativePath = null)
		{
			...
			else
			{
				...
				IsUnitySerialized = hasSerializedProperty || setParent.IsUnitySerialized; //GuessIfIsUnitySerialized(fieldInfo);
		...
		public void Setup([CanBeNull]LinkedMemberInfo setParent, [NotNull]FieldInfo fieldInfo, LinkedMemberParent setParentType, string setSerializedPropertyRelativePath = null)
		{
			...
			else
			{
				...
				// getting this right is pretty critical, because if it's wrong it can break Undo and asset dirtying...
				IsUnitySerialized = hasSerializedProperty || GuessIfIsUnitySerialized(fieldInfo);
		...
		public void Setup([CanBeNull]LinkedMemberInfo setParent, [NotNull]PropertyInfo propertyInfo, LinkedMemberParent setParentType, string setSerializedPropertyRelativePath = null)
		{
			...
			else
			{
				...
				// getting this right is pretty critical, because if it's wrong it can break Undo and asset dirtying...
				IsUnitySerialized = hasSerializedProperty || GuessIfIsUnitySerialized(propertyInfo);
		...
		public T GetValue<T>(int index)
		{
			...
			{
				Debug.LogError(ToString()+".GetValue<"+typeof(T).FullName+">("+index+") Type="+Type.FullName+", resultType="+StringUtils.TypeToString(result)+", result="+StringUtils.ToString(result)+" - " + e);
				throw;
			}
			#else
			try
		...
		{
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(fieldOwner != null || IsStatic, ToString()+".GetValue called with null fieldOwner");
			#endif
		...
		{
			return LinkedMemberType != LinkedMemberType.Method && LinkedMemberType != LinkedMemberType.Parameter && CanRead;
		}
		...
		public bool SetValues([NotNull]Object[] targets, [NotNull]ref object[] fieldOwners, object value)
		{
			...
			Debug.Log(StringUtils.ToColorizedString(ToString()+".SetValues(", fieldOwners, ", val=", value, ") with valueNeedsToBeUpdatedRecursively=", valueNeedsToBeUpdatedRecursively));
			#endif
		...
		public bool SetValues([NotNull]Object[] targets, [NotNull]ref object[] fieldOwners, [NotNull]object[] values)
		{
			...
			#if DEV_MODE && DEBUG_SET_VALUES
			Debug.Log(StringUtils.ToColorizedString("SetValues(owners=", StringUtils.ToString(fieldOwners), ", values=", StringUtils.ToString(values), ") with valueNeedsToBeUpdatedRecursively=", valueNeedsToBeUpdatedRecursively));
			#endif
		...
		[NotNull]
		public object[] GetAttributes(bool inherit = true, bool considerAttributeTarget = true)
		{
		...
		private void UpdateValueNeedsToBeUpdatedRecursively()
		{
			...
			if(LinkedMemberType == LinkedMemberType.CollectionResizer)
			{
				...
				if(mightNeedToToBeUpdatedRecursively && CanReadWithoutSideEffects)
				{
					try
					{
						if((parent.GetValue(0) as IList).IsFixedSize)
						{
							valueNeedsToBeUpdatedRecursively = true;
						}
					}
					...
					{
						#if DEV_MODE
						Debug.LogWarning("(parent.GetValue(0) as IList).IsFixedSize Exception with parnet="+StringUtils.ToString(parent));
						#endif
