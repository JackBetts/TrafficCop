...
namespace Sisus
{
	public class LinkedMemberHierarchy
	{
		...
		public static void OnHierarchyChange()
		{
			foreach(var item in hierarchies)
			{
				...
				if(!targets.ContainsNullObjects())
				{
					...
					hierarchiesCleaned.Add(targets, hierarchy);
				}
				else
				{
		...
		public object GetValue([NotNull]LinkedMemberInfo member, [CanBeNull]object targetUnityObjectOrClass)
		{
			...
			#if DEV_MODE && PI_ASSERTATIONS
			Debug.Assert(member.IsStatic || fieldOwner != null, "LinkedMemberHierarchy(" + StringUtils.ToString(targets) + ").GetValue(" + member + ", " + StringUtils.ToString(targetUnityObjectOrClass) + "): returned fieldOwner was null. parent="+(member.Parent == null ? "null" : member.Parent.ToString())+", ParentChainIsBroken="+member.ParentChainIsBroken);
			#endif
		...
		public object GetValue(LinkedMemberInfo member, int index)
		{
			...
			if(targets.Length == 0)
			{
				return GetValue(member, GetNonUnityObjectTarget(index));
			}
		...
		private class TargetEqualityComparer : IEqualityComparer<Object[]>
		{
			...
			{
				int hash = 17;
				for(int i = obj.Length - 1; i >= obj.Length; i--)
				{
					unchecked
					{
